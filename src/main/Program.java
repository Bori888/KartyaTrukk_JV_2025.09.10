...
public class Program {
    ...
    private static void osszeallit() {
        // 21 lap feltöltése (csak példaértékek)
...
private static void kever(int oszlop) {
    ...
    // Oszlopok kiszedése a pakliból (ahogy a kirakás is használja)
    for (int i = 0; i < 7; i++) {
        oszlop1[i] = pakli[i];
        oszlop2[i] = pakli[i + 7];
        oszlop3[i] = pakli[i + 14];
    }

    // Az új paklit most is oszloponként fogjuk felépíteni!
    String[] ujPakli = new String[21];

    // Új sorrend: választott oszlop mindig középre kerül
    ...
    // Most a kirakáshoz illeszkedve: új pakli oszloponként (7-7-7)
    for (int i = 0; i < 7; i++) {
        ujPakli[i] = bal[i];
        ujPakli[i + 7] = kozep[i];
        ujPakli[i + 14] = jobb[i];
